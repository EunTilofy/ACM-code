...
void dfs(int x,long long now,int up) {
	if(x==si[1]+1) {
		long long z=calc();
		now+=si[1];
//		printf("%lld %lld %d\n",now,z,si[1]);
...
//		for(int i=si[1];i>0&&!c[i];i--)now--;
	...
	if(up) {
		if(a[0][x]<a[1][x])c[x]=a[1][x]-1,dfs(x+1,now+(a[1][x]-1),0);
		c[x]=a[1][x],dfs(x+1,now+a[1][x],1);
	} else c[x]=k-1,dfs(x+1,now+k-1,0);
}
int main() {
	scanf("%d",&T);
	for(e=1; e<=T; e++) {
...
//		for(i=1;i<=si[1];i++)printf("%lld ",a[1][i]); 
		if(si[0]<si[1]) {
			ans=si[1]-1;
